...
| Panel page is blank / black; install says API ready | (1) Caddy proxied `/api/*` to Laravel **without** stripping `/api` (routes are `/v1/*`); (2) frontend `AppProviders` returned `null` until hydrate | Pull latest. Reload Caddy + rebuild frontend: `docker compose --env-file panel/.env -f panel/docker-compose.panel.yml up -d --build --force-recreate web frontend`. Then open `http://SERVER:2090/setup`. Quick check: `curl -sf http://127.0.0.1:2090/api/v1/setup/status` must return JSON (not 404). |
