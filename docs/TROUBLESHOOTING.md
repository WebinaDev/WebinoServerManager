...
| Softstore PHP 8.1/8.3/8.4: `Unable to locate package php8.x-fpm` on **Debian** Bookworm | Distro only ships default PHP (8.2 on Bookworm); other versions need **packages.sury.org** | Pull latest agent — on missing packages it adds Sury PHP repo (Debian) or `ppa:ondrej/php` (Ubuntu), then retries. Softstore apt also runs on the **host** via `nsenter` when the agent is privileged/`pid:host`. Rebuild agent, then retry failed PHP steps. |
| Softstore PHP missing on Ubuntu 24.04 | Distro only ships one PHP; others need **ondrej/php** | Same as above — latest agent adds Ondřej PPA then retries. |
