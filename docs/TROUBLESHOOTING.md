...
| `The route api/v1/setup could not be found` | Same path mismatch: request kept `/api` prefix, Laravel only had `/v1/setup` | Pull latest + recreate **backend** (PHP code). Confirm: `docker compose ... exec backend php artisan route:list --path=setup` shows `api/v1/setup` and `v1/setup`. |
