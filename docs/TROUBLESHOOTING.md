...
| Setup stack: `Package 'mariadb-server' has no installation candidate` | Minimal Ubuntu cloud image without **universe** (or only MySQL metapackage) | Pull latest agent — `install_mariadb` enables universe and falls back to `default-mysql-server` / `mysql-server`. Rebuild agent, then retry failed steps in `/setup/stack`. Manual: `apt-get update && apt-get install -y default-mysql-server` (or enable universe and install `mariadb-server`). |
