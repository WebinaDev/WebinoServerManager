...
| `The route api/v1/setup could not be found` | Browser posts `/api/v1/setup` but image only had module routes at `/v1/setup` | Pull latest (`routes/api.php` registers setup under Laravel `/api` prefix). Rebuild backend: `docker compose --env-file panel/.env -f panel/docker-compose.panel.yml up -d --build --force-recreate backend worker scheduler`. Quick patch without full rebuild: `docker cp panel/backend/routes/api.php webinoserver-backend:/var/www/html/routes/api.php && docker restart webinoserver-backend`. Verify: `curl -sf http://127.0.0.1:2090/api/v1/setup/status`. |
