...
export default function PhpMyAdminPage({ initialDatabaseId }: PhpMyAdminPageProps) {
  const { t } = useTranslation(["phpmyadmin", "databases", "common"])
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("phpmyadmin:title")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap items-end gap-3">
            <div className="space-y-2">
              <Label htmlFor="database_id">{t("phpmyadmin:field_database")}</Label>
              <select
                id="database_id"
              ...
              >
                <option value="">{t("phpmyadmin:no_database")}</option>
                {databases.map((d) => (
          ...
          {iframeSrc ? (
            ...
            <p className="text-muted-foreground text-sm">{t("phpmyadmin:hint")}</p>
          )}
        </CardContent>
      </Card>
    </div>
